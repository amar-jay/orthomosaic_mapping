...
for idx in frame_ids:
	...
	if len(matches[:, 0]) >= 4:
		src = np.float32([keypoints_1[m.queryIdx].pt for m in matches[:, 0]]).reshape(
			-1, 1, 2
		)
		dst = np.float32([keypoints_2[m.trainIdx].pt for m in matches[:, 0]]).reshape(
			-1, 1, 2
		)
	...
	img_1 = cv2.warpPerspective(
		img_1, H, ((img_1.shape[1] + img_2.shape[1]), int(1.2 * img_2.shape[0]))
	)  # wraped image
